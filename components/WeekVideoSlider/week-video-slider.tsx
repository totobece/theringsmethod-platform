...
export default function WeekVideoSlider() {
  ...
  const effectiveStartWeekIndex = 0; 
  if (data.length > 0) { 
    for (let i = effectiveStartWeekIndex; i < data.length; i += 7) {
        ...
        <div className="max-w-full relative">
          <div className="w-full flex justify-center">
            {currentGroupData.length > 0 ? (
              ...
              <div className="text-gray-600 p-4">No routines found for the current week.</div>
            )}
          </div>
        </div>
      )}
